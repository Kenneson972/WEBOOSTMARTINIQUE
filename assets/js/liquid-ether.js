// LiquidEther - Version Complète avec WebGL et Simulations Fluides
...
class LiquidEther {
  constructor(containerId, options = {}) {
    ...
    this.mountRef = { current: this.container };
  ...
  setupContainer() {
    const container = this.mountRef.current;
  ...
  createWebGLManager() {
    ...
    // Simulation Classes (Advection, ExternalForce, Viscous, Divergence, Poisson, Pressure)
  ...
  applyOptions() {
    ...
    const sim = this.webglRef.current.output?.simulation;
    if (!sim) return;
    ...
    const io = new IntersectionObserver(
      entries => {
        ...
        if (isVisible && !document.hidden) {
          ...
          this.webglRef.current.pause();
        }
      },
      { threshold: [0, 0.01, 0.1] }
    );
    io.observe(this.container);
    this.intersectionObserverRef.current = io;
  }

  setupResizeObserver() {
    const ro = new ResizeObserver(() => {
      if (!this.webglRef.current) return;
      if (this.resizeRafRef.current) cancelAnimationFrame(this.resizeRafRef.current);
      this.resizeRafRef.current = requestAnimationFrame(() => {
        if (!this.webglRef.current) return;
        this.webglRef.current.resize();
      });
    });
    ro.observe(this.container);
    this.resizeObserverRef.current = ro;
  }
...
// Initialize LiquidEther when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
  // Check if Three.js is loaded
  if (typeof THREE === 'undefined') {
    console.error('Three.js is not loaded. Please include Three.js before LiquidEther.');
    return;
  }
